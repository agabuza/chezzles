...
namespace chezzles.cocossharp.Extensions
{
    public static class CocoExtensions
    {
        ...
        /// <returns></returns>
        public static T PlaceAt<T>(this T node, double xPct, double yPct, int zIndex, CCNode parent = null) where T : CCNode
        {
        ...
        public static T ScaleToSize<T>(this T node, float width, float height) where T : CCNode
        {
            var size = node.ContentSize;
            node.ScaleX = width / size.Width;
            node.ScaleY = height / size.Height;
            return node;
        }       
